...
local function parsing(id, func)
  local sp = activity.getExternalFilesDir(nil).getPath().."/music/cache/"..tostring(id)..".mp3"
...
function _M.saveMsg(id, msg, isJson)
  ...
  _M.makeSureDir()
  local localPath = activity.getExternalFilesDir(nil).getPath().."/music/cache/"..tostring(id)..".json"
  if isJson then
...
function _M.subMusicById(id)
  ...
  if File(sp).isFile()
    File(sp).delete()
    myToast.toast("对MikuBeat施展了催眠，忘掉了id"..tostring(id).."的歌", 500)
   else
    myToast.toast("可是我本来就不会这首歌啊(ー ー゛)", 500)
  end
end
  ...
  _M.idGetMsg(id,function(msg)
    ...
    view.cover.onLongClick = function()
      Http.download(msg.pic, "/sdcard/Pictures/"..tostring(id)..".jpg",function(code)
        if code == 200 then
          local MediaScannerConnection = bindClass "android.media.MediaScannerConnection"
          MediaScannerConnection.scanFile(activity, {File("/sdcard/Pictures/"..tostring(id)..".jpg").getAbsolutePath()}, {"image/png"}, nil)
